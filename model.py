...
if __name__ == "__main__":
    ...
    # Test forward pass
    dummy_input = torch.randn(8, 1, 3, 500)  # (batch, 1, channels, samples)
    ...
    print("\n" + "=" * 60)
    print("Testing EEGNetTransfer...")
    transfer_model = EEGNetTransfer(model, num_classes=2)
    
    # Count trainable parameters with frozen features
    print(f"Trainable parameters (frozen): {count_parameters(transfer_model):,}")
    
    # Unfreeze and count again
    ...
    print(f"Trainable parameters (last 1 block): {count_parameters(transfer_model):,}")
