...
class Encoder(nn.Module):
    """
    Pure Transformer Encoder for 1D time-series data (e.g., EEG, audio).
    
    This encoder processes 1D sequences, not 2D images.
    Compatible with any time-series data of shape [batch, in_channels, seq_length].
    """
    ...
    def __init__(self, 
        ...
        if embedding_type == 'conv1d':
            ...
            # Segment-based: [B, C, L] -> [B, num_segments, D]
    ...
    def forward(self, src, src_mask=None):
        ...
        Args:
            src: Input tensor of shape [batch_size, in_channels, seq_length]
                 e.g., for EEG: [batch_size, num_electrodes, seq_length]
        ...
        # Input: [B, C, L] -> Output: [B, num_tokens, D]
        x = self.sequence_embedding(src)
        
        # ✅ 2. Add CLS token (if enabled)
        if self.use_cls_token:
            batch_size = src.shape[0]
            cls_tokens = self.cls_token.repeat(batch_size, 1, 1)  # [B, 1, D]
            x = torch.cat([cls_tokens, x], dim=1)  # [B, num_tokens+1, D]
